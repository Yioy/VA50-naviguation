    ...
    def get_traffic_signs(self, img):
        ...
        for out in outs:
            for detection in out:
                ...
                #get Pc => probability of confidence)
                confidence = scores[class_id]
                #get Bw, Bh, Bx, By, C1, C2)
        ...
        #(fst : boxes(x,y,w,h), snd : scores of confidences, thrd : threshold for scores, 4th : non-maximum suppression threshold) 
        # : The last parameter to increase ??
        ...
        for i in range(len(boxes)):
            if i in indexes:
                x, y, w, h = boxes[i]
                #get degree of confidence of the class id (label) for each boxes
                global_label = str(GLOBAL_LABELS[class_ids[i]]) + "=" + str(round(confidences[i]*100, 2)) + "%"
                #crop the images with the values provided by the boxes
                crop_img = img[y:y+h, x:x+w]
                
                if len(crop_img) >0:
                    crop_img = cv.resize(crop_img, (32, 32))
                    #enhancing the contrast of an image by performing adaptive histogram equalization
